...
const home = await mkdtemp(join(tmpdir(), "pulse-opencode-"));
process.env.PULSE_HOME = home;
...
async function runFIFORegression(kind) {
  ...
  const resourceURL = new URL(
    kind === "opencode"
      ? "../../Sources/PulseCore/Resources/opencode/pulse.js"
      : "../../Sources/PulseCore/Resources/pi/pulse.ts",
    import.meta.url,
  ).href;
  ...
  await writeFile(driverURL, `
    ...
    if (${JSON.stringify(kind)} === "opencode") {
      const { PulsePlugin } = await import(${JSON.stringify(resourceURL)});
      ...
      const { default: pulse } = await import(${JSON.stringify(resourceURL)});
  ...
  `);
  return runNodeDriver(driverURL, { PULSE_HOME: childHome });
}

try {
  const { PulsePlugin } = await import(
    "../../Sources/PulseCore/Resources/opencode/pulse.js"
  );
  ...
  const failedEndPlugin = await PulsePlugin({
    directory: "/tmp/project",
    client: {
      session: { get: async () => ({ data: {} }) },
  ...
  const runStalledBurst = async (sessionID, events) => {
    ...
    let lookupCount = 0;
    let stalledSignal;
    const stalledPlugin = await PulsePlugin({
      directory: "/tmp/project",
      client: {
        session: {
          get: async (options) => {
  ...
  const writeFailures = [];
  const failedWritePlugin = await PulsePlugin({
    directory: "/tmp/project",
    client: {
      session: { get: async () => ({ data: {} }) },
  ...
  const { default: pulse } = await import(
    "../../Sources/PulseCore/Resources/pi/pulse.ts"
  );
  const piHandlers = new Map();
  pulse({ on: (name, handler) => piHandlers.set(name, handler) });
