...
func testClaudeHookWrapperForwardsPayloadAndEvent() throws {
    ...
    let binaryURL = directory.appendingPathComponent("atalaia")
    try Data(
        """
        #!/bin/sh
...
func testFocusedWindowFrameWinsOverPointerAndDefaultDisplay() throws {
    // Display B is first to model Atalaia's own main/default screen, while
...
func testOpenCodePluginWritesSessionState() throws {
    ...
    try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: directory) }
    let pluginURL = directory.appendingPathComponent("atalaia.mjs")
    ...
    try Data(
        """
        import { AtalaiaPlugin } from "\(pluginURL.absoluteString)";
...
func testOpenCodePluginMapsLifecycleEvents() throws {
    ...
    try Data(
        ...
        const statuses = [];
        const stateURL = `${process.env.ATALAIA_HOME}/state/opencode-b3Blbi0x.json`;
...
func testPiExtensionWritesSessionState() throws {
    ...
    try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: directory) }
    let extensionURL = directory.appendingPathComponent("atalaia.mjs")
    try FileManager.default.copyItem(at: BundledResources.piExtensionURL, to: extensionURL)
    let driver = directory.appendingPathComponent("driver.mjs")
    try Data(
        """
        import atalaia from "\(extensionURL.absoluteString)";
...
func testPiExtensionMapsLifecycleEvents() throws {
    ...
    try Data(
        ...
        const states = [];
        const stateURL = `${process.env.ATALAIA_HOME}/state/pi-cGktMQ.json`;
...
func testConvoyWatcherSuppressesPipelineOwnedOpenCodeSessions() throws {
    ...
    // if that server loads the Atalaia plugin, each phase would also
...
func spawnFakeAgent(
    ...
    let rootDirectory = fileManager.temporaryDirectory
        .appendingPathComponent("atalaia-scanner-\(UUID().uuidString)", isDirectory: true)
...
func testProcessScannerDetectsScriptRuntimeAgents() throws {
    ...
    let root = fileManager.temporaryDirectory
        .appendingPathComponent("atalaia-runtime-\(UUID().uuidString)", isDirectory: true)
...
func testProcessScannerCollapsesRuntimeLauncherOntoNativeChild() throws {
    ...
    let fileManager = FileManager.default
    let root = fileManager.temporaryDirectory
        .appendingPathComponent("atalaia-launcher-\(UUID().uuidString)", isDirectory: true)
    let binDirectory = root.appendingPathComponent("bin", isDirectory: true)
...
func testClaudeSettingsMergePreservesHooksAndIsIdempotent() throws {
    ...
    let once = try ClaudeSettingsMerger.merge(
        ...
        hookCommand: "/Users/me/.atalaia/bin/claude-hook.sh"
    )
...
func testInstallerRejectsSymlinkedPrivateDirectory() throws {
    ...
    defer { try? FileManager.default.removeItem(at: root) }
    try FileManager.default.createSymbolicLink(
        at: home.appendingPathComponent(".atalaia"),
...
func testInstallerFollowsUserOwnedSymlinkedIntegrationDirectories() throws {
    ...
    // directory under home — only ~/.atalaia is strictly symlink-free.
    let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    let home = root.appendingPathComponent("home")
    ...
    try expect(
        FileManager.default.fileExists(
            atPath: dotfilesPlugins.appendingPathComponent("atalaia.js").path
        ),
        equals: true,
...
func testInstallerReplacesMarkedIntegrationFilesOnReinstall() throws {
    ...
    defer { try? FileManager.default.removeItem(at: root) }
    let outdated = "// Atalaia-managed integration; reinstalls replace this file.\nexport const Old = 1;\n"
    let pluginURL = home.appendingPathComponent(".config/opencode/plugins/atalaia.js")
    ...
    let userExtensionURL = home.appendingPathComponent(".pi/agent/extensions/atalaia.ts")
...
func testInstallationDoctorReportsHealthyInstall() throws {
    ...
    try expect(
        FileManager.default.fileExists(
            atPath: home.appendingPathComponent(".pi/agent/extensions/atalaia.ts").path
        ),
        equals: true,
...
func testInstallationDoctorPinpointsBrokenPieces() throws {
    ...
    try FileManager.default.removeItem(
        at: home.appendingPathComponent(".atalaia/bin/claude-hook.sh")
    )
    try Data("tampered".utf8).write(
        to: home.appendingPathComponent(".config/opencode/plugins/atalaia.js")
    )
    try Data("model = \"gpt-5\"\n".utf8).write(
        to: home.appendingPathComponent(".codex/config.toml")
    )
    try FileManager.default.removeItem(
        at: home.appendingPathComponent(".pi/agent/extensions/atalaia.ts")
    )
...
func testSessionTitleFormatterCleansTabTitles() throws {
    ...
    try expect(
        SessionTitleFormatter.rowTitle(tabTitle: "✳ Atalaia — claude", fallback: "repo"),
        equals: "Atalaia — claude",
    ...
    try expect(
        SessionTitleFormatter.truncate("Atalaia", to: 14),
        equals: "Atalaia",
