...
@main
struct AtalaiaApplication: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var appDelegate

    var body: some Scene {
        Settings {
            AtalaiaSettingsView(store: appDelegate.store)
        }
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let stateDirectory = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".atalaia/state", isDirectory: true)
        ...
        do {
            ...
            guard let lock = try SingleInstanceLock.acquire(
                ...
                NSLog("Atalaia: another instance owns the application lock; exiting.")
                NSApp.terminate(nil)
                return
            }
            instanceLock = lock
        } catch {
            NSLog("Atalaia failed to acquire its application lock: %@", String(describing: error))
            NSApp.terminate(nil)
            return
        }
        ...
        let store = StateStore(
            ...
            nameOverridesFileURL: FileManager.default.homeDirectoryForCurrentUser
                .appendingPathComponent(".atalaia/session-names.json")
        )
        self.store = store
        ...
        scheduler.startWithInitialReconciliation { [weak self, weak scheduler, weak store] in
            ...
            do {
                ...
                NSLog("Atalaia failed to start state observation: %@", String(describing: error))
            }
    ...
    /// `kill -USR1 $(pgrep -x Atalaia)` fotografa a faixa de topo do ecrã
    /// para /tmp/atalaia-shot.png.
    ///
