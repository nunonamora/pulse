...
/// Read-only diagnosis of an Pulse installation. Every check inspects
...
public struct InstallationDoctor {
    ...
    private var binDirectory: URL {
        homeDirectoryURL.appendingPathComponent(".pulse/bin")
    }
    ...
    private func stateDirectoryCheck() -> DoctorCheck {
        let stateDirectory = homeDirectoryURL.appendingPathComponent(".pulse/state")
        ...
        return DoctorCheck(
            ...
            detail: exists && isDirectory.boolValue
                ...
                : "\(display(stateDirectory)) is missing — run: pulse install"
        )
    }
    ...
    private func claudeHooksCheck() -> DoctorCheck {
        ...
        guard let settingsData = try? Data(contentsOf: settingsURL) else {
            return DoctorCheck(
                ...
                passed: false,
                detail: "\(display(settingsURL)) is missing — run: pulse install"
            )
        }
        ...
        return DoctorCheck(
            ...
            detail: installed
                ...
                : "hooks missing from \(display(settingsURL)) — run: pulse install"
        )
    }
    ...
    private func openCodePluginCheck() -> DoctorCheck {
        integrationFileCheck(
            ...
            relativePath: ".config/opencode/plugins/pulse.js",
    ...
    private func piExtensionCheck() -> DoctorCheck {
        integrationFileCheck(
            title: "Pi extension",
            relativePath: ".pi/agent/extensions/pulse.ts",
    ...
    private func integrationFileCheck(
        ...
        guard let installed = try? Data(contentsOf: destination) else {
            return DoctorCheck(
                title: title,
                passed: false,
                detail: "\(display(destination)) is missing — run: pulse install"
            )
        }
    ...
    private func codexNotifyCheck() -> DoctorCheck {
        ...
        guard let config = try? String(contentsOf: configURL, encoding: .utf8) else {
            return DoctorCheck(
                title: "Codex notify",
                passed: false,
                detail: "\(display(configURL)) is missing — run: pulse install"
            )
        }
        ...
        return DoctorCheck(
            ...
            detail: configured
                ...
                : "notify hook missing from \(display(configURL)) — run: pulse install"
        )
    }
