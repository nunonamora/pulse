...
case "$tty_name" in
    ""|"??") PULSE_TTY="" ;;
    /dev/*) PULSE_TTY=$tty_name ;;
    *) PULSE_TTY="/dev/$tty_name" ;;
esac
export PULSE_TTY
...
if [ "$event" = "PermissionRequest" ]; then
    "$script_directory/pulse" hook claude "$event" --pid "$PPID" 2>/dev/null || true
    exit 0
fi

"$script_directory/pulse" hook claude "$event" --pid "$PPID" >/dev/null 2>&1 || true
exit 0
