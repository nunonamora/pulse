// Atalaia-managed integration; reinstalls replace this file.
// Atalaia session-status extension for Pi (https://github.com/badlogic/pi-mono).
// Installed by `atalaia install` into ~/.pi/agent/extensions/. Mirrors the
// OpenCode plugin: every lifecycle event is persisted as a versioned state
// document under ~/.atalaia/state for the notch app to observe.
...
const stateDirectory = join(
  process.env.ATALAIA_HOME || join(homedir(), ".atalaia"),
  "state",
);
...
async function writeState(state) {
  ...
  if (encoded.length > 128) throw new Error("Atalaia session ID exceeds 128 bytes");
  ...
  const notifier = spawn("/usr/bin/notifyutil", ["-p", "com.atalaia.stateChanged"], {
    stdio: "ignore",
  });
  notifier.on("error", () => {});
  notifier.unref();
...
function guarded(handler) {
  return async (event, ctx) => {
    try {
      ...
      if (!warnedOnce) {
        warnedOnce = true;
        console.error(`atalaia: state update failed: ${String(error)}`);
      }
    }
  };
}

export default function atalaia(pi) {
