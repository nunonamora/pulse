...
public struct Installer {
    ...
    public func install() throws {
        let atalaiaDirectory = homeDirectoryURL.appendingPathComponent(".atalaia")
        let binaryDirectory = atalaiaDirectory.appendingPathComponent("bin")
        try validatePrivateDirectory(atalaiaDirectory)
        try validateIntegrationDirectories(Self.integrationDirectories(under: homeDirectoryURL))
        try preflightInstallation()
        try FileManager.default.createDirectory(
            at: atalaiaDirectory.appendingPathComponent("state"),
            withIntermediateDirectories: true
        )
        try FileManager.default.createDirectory(at: binaryDirectory, withIntermediateDirectories: true)
        for directory in [atalaiaDirectory, binaryDirectory, atalaiaDirectory.appendingPathComponent("state")] {
            try FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: directory.path)
        }
        try copy(executableURL, to: binaryDirectory.appendingPathComponent("atalaia"), executable: true)
    ...
    public func uninstall() throws {
        ...
        try uninstallClaudeSettings(
            hookDirectory: atalaiaDirectory.appendingPathComponent("bin")
        )
        try uninstallCodexNotify(
            hookDirectory: atalaiaDirectory.appendingPathComponent("bin")
        )
        try removeOwnedIntegrationFile(
            at: homeDirectoryURL.appendingPathComponent(".config/opencode/plugins/atalaia.js"),
            matching: BundledResources.opencodePluginURL
        )
        try removeOwnedIntegrationFile(
            at: homeDirectoryURL.appendingPathComponent(".pi/agent/extensions/atalaia.ts"),
            matching: BundledResources.piExtensionURL
        )
        try removeIfPresent(atalaiaDirectory)
    }
    ...
    static let integrationOwnershipMarker = "Atalaia-managed integration"

    private static func isAtalaiaOwned(_ contents: Data) -> Bool {
    ...
    private func mayReplaceIntegrationFile(at destination: URL, bundled: URL) throws -> Bool {
        ...
        return existing == (try Data(contentsOf: bundled)) || Self.isAtalaiaOwned(existing)
    }
    ...
    private func preflightInstallation() throws {
        ...
        if FileManager.default.fileExists(atPath: claudeSettings.path) {
            _ = try ClaudeSettingsMerger.merge(
                ...
                hookCommand: homeDirectoryURL.appendingPathComponent(".atalaia/bin/claude-hook.sh").path
            )
        }
        ...
        let integrationFiles: [(destination: String, bundled: URL)] = [
            (".config/opencode/plugins/atalaia.js", BundledResources.opencodePluginURL),
            (".pi/agent/extensions/atalaia.ts", BundledResources.piExtensionURL),
        ]
